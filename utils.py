...
def make_search_string(df, league, top_n, fam, iv_b, inv_b, sho_xl_val,sho_only_xl, all_pre=False,shad_only=False, language='English'):
    if league == 'little':
        ...
        return get_top_50_ids(df, 'Master_Rank', 'master', top_n, fam, iv_b, inv_b, True, sho_only_xl, all_pre,shad_only, language)
    elif league == 'all':
        if not inv_b:
            return (
                ...
                + ','
                + get_top_50_ids(df, 'Master_Rank', 'master', top_n, fam, iv_b, inv_b, True, sho_only_xl, all_pre,shad_only, language)
            )
        else:
            return (
                get_top_50_ids(df, 'Little_Rank', 'little', top_n, fam, iv_b, inv_b, sho_xl_val,sho_only_xl, all_pre,shad_only, language)
